import {Box, Text, Heading, Avatar, VStack, useMediaQuery, Flex} from '@chakra-ui/react';
...
const Testimonials = () => {
  ...
  return (
    <Box mx="auto" py={16} px={{ base: 4, md: 8 }} maxW="1440px" textAlign="center">
      <Heading fontFamily="Bevan" fontWeight="700" fontSize={{ base: "32px", md: "48px" }} color="#18171C" mb={12}>
        Voices of Hope
      </Heading>
      <Carousel showArrows={true} showIndicators={false} showThumbs={false} showStatus={false} infiniteLoop={true} autoPlay={true}>
        {testimonials.map((testimonial, index) => (
          <Box key={index} p={{ base: 4, md: 8 }} mx="auto" maxW="800px">
            <Flex direction={{ base: "column", md: "row" }} alignItems="center" justifyContent="center" textAlign={{ base: 'center', md: 'left' }} gap={8}>
              <Avatar src={testimonial.image} size="xl" mb={{ base: 4, md: 0 }} />
              <VStack align="left" spacing={4}>
                <Text fontFamily="Epilogue" fontWeight="400" fontSize="16px" color="#4F4F4F">
                  {testimonial.text}
                </Text>
                <Text fontFamily="Epilogue" fontWeight="700" fontSize="16px" color="#18171C">
